...
Page({
    ...
    if (ThisIndex < 3) {
      ...
      app.post('user/sign', { userId: app.globalData.userId },
        function (res) {
          if (res.code == 1000) {
            var source=0;
            if (app.globalData.userInfo.source)
            {
              source = app.globalData.userInfo.source+2;
              app.globalData.userInfo.source+=2;
            }
             that.setData({
               showdetail:true,
               signDetail:res.body,
             ...
             app.post('user/source', { type: 1, number: 2, userId: app.globalData.userInfo.sunwouId },function(res){

             })
          }else{
            var source = 0;
            if (app.globalData.userInfo.source)
              source = app.globalData.userInfo.source;
            that.setData({
              showdetail: true,
              signDetail: res.body,
              source: source
  ...
  findImage: function () {
    app.post('shop/findimageandtext', { type: 1,appid: app.sunwouId}, function (res) {
      that.setData({
        Images: res.body
      })
    })
    ...
    function (res) {
      console.log(res)
       if(res.body.length==3)
       {
         ...
         for(var i=0;i<3;i++)
         {
           that.data.index_item[i].text = res.body[i].name;
  
         }
       }
       that.setData({
         index_item: that.data.index_item
       })
    })
  },
